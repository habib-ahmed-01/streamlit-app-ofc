...
async def insert_data(pool, data):
    async with pool.acquire() as connection:
        query = """
        INSERT INTO ic_ilos (fqdn, ip, region, version)
        VALUES ($1, $2, $3, $4)
        """
        await connection.execute(query, data['fqdn'], data['ip'], data['region'], data['version'])


# Deletes the rows that were selected by the user
async def delete_data(pool, data):
    async with pool.acquire() as connection:
        query = """Delete from ic_ilos where id = $1"""
